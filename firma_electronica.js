...
/* ============================================================
   AUROSANAX FIRMA ELECTRÓNICA V3.1
   PUENTE LOCAL DE IDENTIDAD DE EQUIPO — CORRECCIÓN QUIRÚRGICA
   ------------------------------------------------------------
   - Adhesión append-only sobre V3.0: baseline anterior intacto.
   - Obtiene id_equipo exclusivamente del motor LOCAL /health.
   - No cambia Adobe, doble firma, polling, REABRIR, CANCELAR,
     PDF persistente, SHA ni contratos clínicos.
   - Si no puede identificar el motor local, falla cerrado ANTES
     de crear una solicitud que pudiera ser tomada por otro PC.
============================================================ */
(function auroFirmaPuenteEquipoLocalV31(){
  'use strict';

  const anterior = window.auroFirmaElectronica;
  if(!anterior || typeof anterior.firmarDocumento !== 'function'){
    console.error('AUROSANAX FIRMA V3.1: no se encontró la API V3.0 previa.');
    return;
  }

  const VERSION = '3.1-equipo-local-dirigido';
  const HEALTH_URLS = [
  ...
  async function consultarHealth_(url){
    ...
    try{
      ...
      if(!json || json.success !== true || !idEquipo) return null;

      return {
        id_equipo:idEquipo,
        version:texto(json.version),
        servicio:texto(json.servicio)
      };
    }catch(_e){
      return null;
    }finally{
  ...
  async function obtenerEquipoLocal_(){
    for(const url of HEALTH_URLS){
      const health = await consultarHealth_(url);
      if(health && health.id_equipo) return health;
    }
    return null;
  }

  async function firmarDocumento(data){
    const health = await obtenerEquipoLocal_();

    if(!health || !texto(health.id_equipo)){
      throw new Error(
        'No se pudo identificar el computador de origen para dirigir la firma. ' +
        'Verifique que el motor local esté iniciado.'
      );
    }

    /*
      Se envían ambos nombres por compatibilidad controlada.
      validarSolicitud() de V2.7/V3.0 conserva campos adicionales,
      por lo que llegan al POST firmarDocumento sin alterar el HTML
      ni la identidad clínica/documental.
    */
    const dirigido = Object.assign({}, data || {}, {
      id_equipo_destino:health.id_equipo,
      id_equipo:health.id_equipo
    });
