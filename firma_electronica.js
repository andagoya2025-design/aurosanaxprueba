  ...
  */
  ...
  async function auroRecetaFirmarElectronicaActual(){
    ...
    try{
      ...
      if(String(resultado?.estado_firma || '').trim().toUpperCase() !== 'FIRMADO'){
        throw new Error('El servidor no confirmó el estado FIRMADO de la receta.');
      }

      resultadoFirmado = resultado;
      firmaConfirmada = true;

      if(
        typeof window.auroFirmaElectronica.abrirPdfFirmado === 'function' &&
        !window.auroFirmaElectronica.abrirPdfFirmado(resultado, documento.nombre_archivo)
      ){
        mostrarMensajeReceta(
          '<i class="bi bi-info-circle me-1"></i> La receta quedó firmada, pero el navegador bloqueó la apertura del PDF. Pulse “Firmada ✓” para verlo.',
          'ok'
        );
      }else{
        mostrarMensajeReceta(
      ...
      if(btn){
        ...
        if(firmaConfirmada && resultadoFirmado){
          btn.disabled = false;
          btn.innerHTML = '<i class="bi bi-patch-check-fill"></i> Firmada ✓';
          btn.title = 'Ver PDF firmado';
          btn.onclick = function(){
            if(
              !window.auroFirmaElectronica ||
              typeof window.auroFirmaElectronica.abrirPdfFirmado !== 'function' ||
              !window.auroFirmaElectronica.abrirPdfFirmado(resultadoFirmado, documento.nombre_archivo)
            ){
              mostrarMensajeReceta(
                '<i class="bi bi-shield-exclamation me-1"></i> No fue posible abrir el PDF firmado.',
                'error'
              );
            }
          };
          ...
          btn.onclick = auroRecetaFirmarElectronicaActual;
        }
      }
    }
  }
