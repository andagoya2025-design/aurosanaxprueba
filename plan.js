    ...
    ****************************************************************/
    ...
    function auroPlanInstalarAccesoViaLibreRapido(){
        ...
        if(!boton){
            ...
            boton.title = 'Mostrar u ocultar una vía diferente';
            boton.innerHTML = '<i class="bi bi-pencil-square me-1"></i> Otra vía';

            boton.addEventListener('click', function(){
                auroPlanAlternarEntradaViaLibre();
            });
    ...
    function auroPlanAlternarEntradaViaLibre(){
        const entrada = auroPlanInstalarEntradaViaLibre();
        const boton = document.getElementById('auroPlanBtnOtraViaRapida');
        if(!entrada) return;

        const visible = !entrada.classList.contains('d-none');
    ...
    function auroPlanAjustarIndicacionesMedicamento(expandido){
        const campo = document.getElementById('hcMedIndicaciones');
        if(!campo || campo.tagName !== 'TEXTAREA') return;

        const alturaCompacta = 44;
        const alturaMaxima = 132;

        if(expandido){
            campo.style.height = 'auto';
            const altura = Math.min(
                Math.max(campo.scrollHeight || alturaCompacta, alturaCompacta),
                alturaMaxima
            );
            campo.style.height = altura + 'px';
            campo.style.overflowY =
                (campo.scrollHeight || 0) > alturaMaxima ? 'auto' : 'hidden';
            campo.classList.add('auro-plan-indicaciones-expandida');
            return;
        }

        campo.style.height = alturaCompacta + 'px';
        campo.style.overflowY =
            (campo.scrollHeight || 0) > alturaCompacta ? 'auto' : 'hidden';
        campo.classList.remove('auro-plan-indicaciones-expandida');
    }

    function auroPlanInstalarIndicacionesAutoExpandibles(){
        const campo = document.getElementById('hcMedIndicaciones');
        if(!campo || campo.tagName !== 'TEXTAREA') return;

        if(campo.dataset.auroAutoExpand === '1'){
            auroPlanAjustarIndicacionesMedicamento(
                document.activeElement === campo
            );
            return;
        }

        campo.dataset.auroAutoExpand = '1';
        campo.classList.add('auro-plan-indicaciones-controladas');

        campo.addEventListener('focus', function(){
            auroPlanAjustarIndicacionesMedicamento(true);
        });
        ...
        campo.addEventListener('blur', function(){
            auroPlanAjustarIndicacionesMedicamento(false);
        });
    ...
    */
    ...
    /* ============================================================
       ...
       - Mantiene compatibilidad total con registros planos históricos.
       - NO cambia la estructura persistida del medicamento.
       - NO modifica id_atencion, guardado, Recetas ni protocolos.
    ============================================================ */

    window.auroPlanCatalogoMedicamentoActivo = null;

    function auroPlanCatalogoBaseSeguro(){
        return Array.isArray(window.MEDICAMENTOS_AUROSANAX_BASE)
            ? window.MEDICAMENTOS_AUROSANAX_BASE
            : [];
    }
        ...
        */
        ...
        if(variantes.length){
            return variantes.map(v => ({
                forma_farmaceutica: String(v.forma_farmaceutica || '').trim(),
                concentracion: String(v.concentracion || '').trim(),
                pres: String(
                    v.pres ||
                    v.presentacion ||
                    [v.concentracion, v.forma_farmaceutica].filter(Boolean).join(' ') ||
                    ''
                ).trim(),
                vias_compatibles: Array.isArray(v.vias_compatibles)
                    ? v.vias_compatibles.map(x => String(x || '').trim()).filter(Boolean)
                    : (v.via ? [String(v.via).trim()] : [])
            }));
        }

        const pres = String(item.pres || '').trim();
        const via = String(item.via || '').trim();

        return (pres || via) ? [{
            forma_farmaceutica: String(item.forma_farmaceutica || '').trim(),
            concentracion: String(item.concentracion || '').trim(),
            pres: pres,
            vias_compatibles: via ? [via] : []
        }] : [];
    }

    function auroPlanBuscarItemCatalogoExacto(med, pres){
        const nMed = normalizarMedTexto(med);
        const nPres = normalizarMedTexto(pres);

        const candidatos = auroPlanCatalogoBaseSeguro().filter(item =>
            normalizarMedTexto(item?.med) === nMed
        );

        if(!candidatos.length) return null;

        if(nPres){
            const exactoPlano = candidatos.find(item =>
                normalizarMedTexto(item?.pres) === nPres
            );
            if(exactoPlano) return exactoPlano;

            const exactoVariante = candidatos.find(item =>
                auroPlanVariantesMedicamento(item).some(v =>
                    normalizarMedTexto(v.pres) === nPres
                )
            );
            if(exactoVariante) return exactoVariante;
        }

        return candidatos[0] || null;
    }

    function auroPlanInstalarPresentacionesDisponibles(item, presentacionPreferida){
        const campo = document.getElementById('hcMedPresentacion');
        if(!campo || campo.tagName !== 'INPUT') return;

        const variantes = auroPlanVariantesMedicamento(item);
        const presentaciones = Array.from(new Set(
            variantes.map(v => String(v.pres || '').trim()).filter(Boolean)
        ));

        let lista = document.getElementById('auroPlanPresentacionesMedicamentoLista');

        if(!lista){
            lista = document.createElement('datalist');
            lista.id = 'auroPlanPresentacionesMedicamentoLista';
            document.body.appendChild(lista);
        }

        lista.innerHTML = presentaciones
            .map(p => `<option value="${escapeHtmlPlan(p)}"></option>`)
            .join('');
        ...
        const preferida = String(presentacionPreferida || '').trim();

        if(preferida){
            auroPlanSetValue('hcMedPresentacion', preferida);
        }else if(presentaciones.length === 1){
            auroPlanSetValue('hcMedPresentacion', presentaciones[0]);
        }
    }
    ...
    function auroPlanRestaurarTodasLasVias(valorPreferido){
        ...
        if(preferido){
            ...
            if(!existe){
                ...
                op.value = preferido;
                op.textContent = auroPlanNombreViaCompleta(preferido);
                campo.appendChild(op);
            }
            campo.value = preferido;
        }
    }
    ...
    function auroPlanAplicarViasCompatibles(item, presentacion){
        ...
        if(nPres){
            variante = variantes.find(v => normalizarMedTexto(v.pres) === nPres) || null;
        }

        if(!variante && variantes.length === 1){
    ...
    function auroPlanActivarMedicamentoCatalogo(item, presentacionPreferida){
        window.auroPlanCatalogoMedicamentoActivo = item || null;

        if(!item){
            const campoPres = document.getElementById('hcMedPresentacion');
            if(campoPres) campoPres.removeAttribute('list');
            auroPlanRestaurarTodasLasVias(auroPlanGetValue('hcMedVia'));
            return;
        }

        auroPlanInstalarPresentacionesDisponibles(item, presentacionPreferida);

        const presActual = auroPlanGetValue('hcMedPresentacion');
        auroPlanAplicarViasCompatibles(item, presActual);
    }

    function auroPlanActualizarViasPorPresentacion(){
        const item = window.auroPlanCatalogoMedicamentoActivo;
        if(!item) return;

        auroPlanAplicarViasCompatibles(
            item,
            auroPlanGetValue('hcMedPresentacion')
        );
    }
    ...
    function seleccionarMedicamentoSugerido(el){
        ...
        const itemCatalogo = auroPlanBuscarItemCatalogoExacto(
            el.dataset.med || '',
            el.dataset.pres || ''
        );
    ...
    function limpiarFormularioMedicamento(opciones){
        ...
        const campoPresentacion = document.getElementById('hcMedPresentacion');
    ...
    function editarMedicamentoPlan(i){
        ...
        /*
          ...
          se conserva exactamente la presentación y vía ya guardadas.
          Si el catálogo actual reconoce el medicamento, solo se ofrecen
          variantes compatibles sin sobrescribir el dato clínico existente.
        */
        const itemCatalogo = auroPlanBuscarItemCatalogoExacto(m.med || '', m.pres || '');
        window.auroPlanCatalogoMedicamentoActivo = itemCatalogo || null;
        auroPlanInstalarPresentacionesDisponibles(itemCatalogo, m.pres || '');
        auroPlanRestaurarTodasLasVias(m.via || '');
        ...
        if(!meds.length){
            ...
            tbody.innerHTML = meds.map((m,i) => `
                <tr>
                  ...
                  <td>
                    <div class="auro-plan-indicacion-tabla"
                         title="${escapeHtmlPlan(m.ind)}">${escapeHtmlPlan(m.ind)}</div>
                  </td>
    ...
    */
    ...
    function instalarEventosMedicamentosPlan(){
        ...
        document.addEventListener('change', function(e){
            ...
            if(e.target?.id === 'hcMedPresentacion'){
    ...
    function instalarResponsivePlanAndroid(){
        ...
        style.textContent = `
          ...
          #hc_plan .auro-plan-indicacion-tabla{
            display:-webkit-box;
    ...
    /* ============================================================
       ...
       - Usa reintentos ACOTADOS tras FIRMADO para esperar que
         la persistencia recién archivada sea visible en la consulta.
       - Al abrir/cambiar atención, espera de forma ACOTADA a que Recetas
         exponga el documento actual antes de resolver el estado de firma.
    ============================================================ */
    (function auroPlanEstadosFirmaV375(){
      ...
      function botonRecetaPlan(){
        return document.querySelector(
          '#auroPlanActionButtons [data-auro-receta-plan-btn="1"], ' +
          '[data-auro-receta-plan-btn="1"]'
        );
      }

      function estaEditandoReceta(){
        const b = botonRecetaPlan();
        if(!b) return false;
      ...
      function pintarEdicion(){
        ...
        b.style.cursor = 'not-allowed';
        b.title = 'Guarde primero la corrección antes de firmar una nueva versión';
      ...
      async function obtenerEstadoPersistente(){
        if(
          !window.auroRecetas ||
          typeof window.auroRecetas.obtenerDocumentoFirmableActual !== 'function' ||
      ...
      function limpiarReintentoFirmado(){
        if(timerFirmado){
          clearTimeout(timerFirmado);
          timerFirmado = null;
        }
        reintentoFirmado = 0;
      }

      function limpiarReintentoCarga(){
        if(timerCarga){
      ...
      function idAtencionActivaFirmaPlan(){
        try{
      ...
      /*
        Entrada/cambio de atención:
        Recetas puede terminar su carga unos instantes después de Plan.
        Se espera como máximo 8 x 500 ms y se invalida automáticamente
        si el usuario cambia nuevamente de atención.
      */
      async function sincronizarCargaAtencion(idEsperado){
        if(operativo) return;

        const esperado = String(
          idEsperado || idAtencionCarga || idAtencionActivaFirmaPlan()
        ).trim();
        const actual = idAtencionActivaFirmaPlan();

        if(esperado && actual && esperado !== actual){
          limpiarReintentoCarga();
          return;
        }
        ...
        reintentoCarga += 1;
        if(reintentoCarga >= MAX_REINTENTOS_CARGA){
          limpiarReintentoCarga();
          if(!operativo) pintarRecetaNoGuardada();
          return;
        }

        timerCarga = setTimeout(function(){
          sincronizarCargaAtencion(esperado);
        }, ESPERA_REINTENTO_MS);
      }

      function iniciarSincronizacionCargaAtencion(idEsperado){
        limpiarReintentoCarga();
        idAtencionCarga = String(
          idEsperado || idAtencionActivaFirmaPlan()
        ).trim();

        timerCarga = setTimeout(function(){
          sincronizarCargaAtencion(idAtencionCarga);
        }, 0);
      }
      ...
      */
      async function confirmarFirmadoPersistente(){
        ...
        try{
          ...
          if(r && r.success && r.estado === 'FIRMADA'){
        ...
        reintentoFirmado += 1;
        if(reintentoFirmado >= MAX_REINTENTOS_FIRMADO){
          limpiarReintentoFirmado();
          return;
        }

        timerFirmado = setTimeout(confirmarFirmadoPersistente, ESPERA_REINTENTO_MS);
      }
      ...
      */
      ...
      window.addEventListener('aurosanax:firma-electronica-estado', function(ev){
        ...
        if(estado === 'CANCELADA' || estado === 'ERROR' || estado === 'NORMAL'){
          pintarOperativo('NORMAL');
      ...
      */
      ...
      function sincronizarFirmaPlanDesdeEventoAtencion(ev){
        const detalle = ev && ev.detail && typeof ev.detail === 'object'
          ? ev.detail
          : {};
        ...
        const idEvento = String(
          detalle.id_atencion ||
          detalle.idAtencion ||
          idAtencionActivaFirmaPlan() ||
          ''
        ).trim();

        iniciarSincronizacionCargaAtencion(idEvento);
      }

      window.addEventListener('aurosanax:atencion-iniciada', sincronizarFirmaPlanDesdeEventoAtencion);
      window.addEventListener('aurosanax:atencion-seleccionada', sincronizarFirmaPlanDesdeEventoAtencion);
      window.addEventListener('aurosanax:atencion-cambiada', sincronizarFirmaPlanDesdeEventoAtencion);

      window.auroPlanSincronizarEstadoFirmaReceta = sincronizar;

      if(document.readyState === 'loading'){
        document.addEventListener('DOMContentLoaded', function(){
          iniciarSincronizacionCargaAtencion();
        }, {once:true});
      }else{
        requestAnimationFrame(function(){
          iniciarSincronizacionCargaAtencion();
