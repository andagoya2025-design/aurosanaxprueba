...
/* ============================================================
   AUROSANAX PLAN - RESTABLECIMIENTO SEGURO POR ATENCIÓN v23
   INTERVENCIÓN AISLADA:
   - NO modifica guardarPlanClinicoDesdeSheets().
   - NO modifica guardarPlanClinicoConUX().
   - NO intercepta ni reemplaza el botón Actualizar Plan Clínico.
   - Solo permite restablecer el Plan de la atención visible/activa.
   - Bloquea restablecimiento en atenciones finalizadas.
   - No elimina ni modifica recetas ya guardadas o emitidas.
============================================================ */
(function instalarRestablecimientoSeguroPlanV23(){
    'use strict';

    if(window.__auroPlanRestablecerSeguroV23Instalado) return;
    window.__auroPlanRestablecerSeguroV23Instalado = true;

    const ID_BOTON = 'auroPlanBtnRestablecerSeguro';
    const ID_DESHACER = 'auroPlanBtnDeshacerRestablecimiento';
    const ID_MODAL = 'auroPlanModalRestablecerSeguro';
    const ID_STYLE = 'auroPlanRestablecerSeguroCSS';

    window.__auroPlanRespaldoRestablecimiento = null;
    ...
    function idAtencionSegura(){
        try{
            if(typeof window.auroPlanObtenerIdAtencionActivaSeguro === 'function'){
                return String(window.auroPlanObtenerIdAtencionActivaSeguro() || '').trim();
            }
        }catch(e){}

        try{
            if(typeof auroPlanObtenerIdAtencionActivaSeguro === 'function'){
                return String(auroPlanObtenerIdAtencionActivaSeguro() || '').trim();
            }
        }catch(e){}

        return String(window.planState?.atencionActual || '').trim();
    }

    function obtenerAtencionPorId(idAtencion){
        idAtencion = String(idAtencion || '').trim();
        if(!idAtencion) return null;
        ...
        try{
            if(typeof window.getAtencionActiva === 'function'){
                ...
                if(activa && String(activa.id_atencion || '').trim() === idAtencion){
                    return activa;
                }
            }
        }catch(e){}
        ...
        try{
            ...
            if(Array.isArray(lista)){
                return lista.find(a =>
                    String(a?.id_atencion || '').trim() === idAtencion
                ) || null;
            }
        }catch(e){
            console.warn('AUROSANAX PLAN: no se pudo consultar el estado de la atención.', e);
        }

        return null;
    }

    function estadoAtencion(idAtencion){
        const atencion = obtenerAtencionPorId(idAtencion);
        return String(
    ...
    function atencionFinalizada(idAtencion){
        const estado = textoNormalizado(estadoAtencion(idAtencion));
    ...
    function recetaGuardadaParaAtencion(idAtencion){
        try{
            const raw = localStorage.getItem('aurosanax_recetas_emitidas_v1');
            const lista = raw ? JSON.parse(raw) : [];
            return Array.isArray(lista) && lista.some(r =>
                String(r?.id_atencion || '').trim() === String(idAtencion || '').trim()
            );
        }catch(e){
            return false;
        }
    ...
    function contextoCoincidente(idAtencion){
        const activa = String(idAtencion || '').trim();
        const interna = String(window.planState?.atencionActual || '').trim();
        const renderizada = String(window.__auroPlanAtencionRenderizada || '').trim();

        if(!activa) return false;
        if(interna && interna !== activa) return false;
        if(renderizada && renderizada !== activa) return false;
        return true;
    }
    ...
    function capturarPlanVisible(idAtencion){
        return {
            id_atencion: String(idAtencion || '').trim(),
            medicamentos: clonar(window.medicamentosPlanSeleccionados || []),
            ordenes: clonar(window.ordenesMedicasPlanSeleccionadas || []),
            interconsultas: clonar(window.interconsultasPlanSeleccionadas || []),
            plan: auroPlanGetValue('hcPlanTratamiento'),
            indicaciones: auroPlanGetValue('hcIndicacionesPaciente'),
            recetaPreparada: auroPlanGetValue('hcRecetaMedicamentos'),
            ordenesTexto: auroPlanGetValue('hcExamenesSolicitados'),
            interconsultaTexto: auroPlanGetValue('hcInterconsultaResumen'),
            evaluacionesTexto: auroPlanGetValue('hcEvaluacionesResumen'),
            evaluacionesChecks: typeof auroPlanCapturarEvaluaciones === 'function'
                ? auroPlanCapturarEvaluaciones()
                : {},
            proximoControl: auroPlanGetValue('hcControl'),
            recetaFormulario: {
                medicamento: auroPlanGetValue('recMedicamento'),
                indicaciones: auroPlanGetValue('recIndicaciones'),
                recomendaciones: auroPlanGetValue('recRecomendaciones')
            }
        };
    }

    function restaurarPlanVisible(respaldo){
        if(!respaldo) return false;

        const idActual = idAtencionSegura();
        if(!idActual || idActual !== String(respaldo.id_atencion || '').trim()){
            alert('No se puede deshacer porque cambió la atención activa.');
            return false;
        }

        if(atencionFinalizada(idActual)){
            alert('Esta atención está finalizada. El Plan no puede restablecerse ni deshacerse.');
            return false;
        }

        window.medicamentosPlanSeleccionados = clonar(respaldo.medicamentos || []);
        window.ordenesMedicasPlanSeleccionadas = clonar(respaldo.ordenes || []);
        window.interconsultasPlanSeleccionadas = clonar(respaldo.interconsultas || []);

        auroPlanSetValue('hcPlanTratamiento', respaldo.plan || '');
        auroPlanSetValue('hcIndicacionesPaciente', respaldo.indicaciones || '');
        auroPlanSetValue('hcRecetaMedicamentos', respaldo.recetaPreparada || '');
        auroPlanSetValue('hcExamenesSolicitados', respaldo.ordenesTexto || '');
        auroPlanSetValue('hcInterconsultaResumen', respaldo.interconsultaTexto || '');
        auroPlanSetValue('hcEvaluacionesResumen', respaldo.evaluacionesTexto || '');
        auroPlanSetValue('hcControl', respaldo.proximoControl || '');

        auroPlanSetValue('recMedicamento', respaldo.recetaFormulario?.medicamento || '');
        auroPlanSetValue('recIndicaciones', respaldo.recetaFormulario?.indicaciones || '');
        auroPlanSetValue('recRecomendaciones', respaldo.recetaFormulario?.recomendaciones || '');

        if(typeof auroPlanRestaurarEvaluaciones === 'function'){
            auroPlanRestaurarEvaluaciones(respaldo.evaluacionesChecks || {});
        }
        ...
        window.__auroPlanRespaldoRestablecimiento = null;
        actualizarBotonesRestablecimiento();
        return true;
    }

    function limpiarSoloPlanActivo(idAtencion){
        idAtencion = String(idAtencion || '').trim();
        if(!contextoCoincidente(idAtencion)){
            alert('Se bloqueó la limpieza porque el Plan visible no corresponde a la atención activa.');
            return false;
        }

        if(atencionFinalizada(idAtencion)){
            alert('Esta atención está finalizada. El Plan no puede restablecerse.');
            return false;
        }

        window.__auroPlanRespaldoRestablecimiento = capturarPlanVisible(idAtencion);

        /* Limpieza exclusivamente visual/temporal del Plan activo. */
        if(typeof limpiarPlanTemporal === 'function'){
            limpiarPlanTemporal();
        }else{
            window.medicamentosPlanSeleccionados = [];
            window.ordenesMedicasPlanSeleccionadas = [];
        ...
        /* limpiarPlanTemporal no incluye próximo control en la versión estable. */
        auroPlanSetValue('hcControl', '');
    ...
    function abrirModalRestablecer(){
        const idAtencion = idAtencionSegura();

        if(!idAtencion){
            alert('No existe una atención activa para restablecer el Plan.');
            return;
        }

        if(!contextoCoincidente(idAtencion)){
            alert('Se bloqueó la acción porque la atención activa no coincide con el Plan visible.');
            return;
        }
        ...
        const existeReceta = recetaGuardadaParaAtencion(idAtencion);
        const modal = document.createElement('div');
        modal.id = ID_MODAL;
        modal.className = 'auro-plan-reset-modal';
        modal.innerHTML = `
          <div class="auro-plan-reset-dialog" role="dialog" aria-modal="true" aria-labelledby="auroPlanResetTitle">
            <div class="auro-plan-reset-icon"><i class="bi bi-arrow-counterclockwise"></i></div>
            <h5 id="auroPlanResetTitle">Restablecer plan clínico</h5>
            <p>Se limpiará únicamente el Plan de esta atención.</p>
            ${existeReceta ? '<div class="auro-plan-reset-warning"><i class="bi bi-exclamation-triangle-fill"></i><span>La receta guardada no se eliminará. Para cambiarla, deberá actualizarla desde Recetas.</span></div>' : ''}
            <div class="auro-plan-reset-id">Atención protegida: ${escapeHtmlPlan(idAtencion)}</div>
            <div class="auro-plan-reset-actions">
              <button type="button" class="btn btn-outline-secondary" data-accion="cancelar">Cancelar</button>
              <button type="button" class="btn btn-danger" data-accion="limpiar"><i class="bi bi-trash3 me-1"></i> Limpiar solo el Plan</button>
            </div>
          </div>`;

        document.body.appendChild(modal);

        modal.querySelector('[data-accion="cancelar"]')?.addEventListener('click', cerrarModal);
        modal.querySelector('[data-accion="limpiar"]')?.addEventListener('click', function(){
            const ok = limpiarSoloPlanActivo(idAtencion);
            cerrarModal();
            if(ok){
                const aviso = existeReceta
                    ? 'Plan restablecido. La receta guardada permanece sin cambios.'
                    : 'Plan restablecido para esta atención.';
                if(typeof window.auroPlanMostrarEstadoGuardado === 'function'){
                    window.auroPlanMostrarEstadoGuardado(aviso);
                }
            }
        });
    ...
    function buscarBotonActualizarPlan(){
        const candidatos = Array.from(document.querySelectorAll('#hc_plan button, button'));
        return candidatos.find(btn => {
            if(btn.id === ID_BOTON || btn.id === ID_DESHACER) return false;
            const onclick = String(btn.getAttribute('onclick') || '');
            const texto = textoNormalizado(btn.textContent || '');
    ...
    function instalarEstilos(){
        ...
        style.id = ID_STYLE;
        style.textContent = `
          .auro-plan-reset-actions-wrap{display:flex;align-items:center;justify-content:flex-end;gap:10px;flex-wrap:wrap;margin-top:10px}
          .auro-plan-reset-actions-wrap .btn{min-height:40px;border-radius:10px;font-weight:700;white-space:nowrap}
          #${ID_BOTON}:disabled{opacity:.55;cursor:not-allowed}
          .auro-plan-reset-modal{position:fixed;inset:0;z-index:100000;display:flex;align-items:center;justify-content:center;padding:18px;background:rgba(15,23,42,.58);backdrop-filter:blur(2px)}
          .auro-plan-reset-dialog{width:min(520px,100%);background:#fff;border:1px solid #e5e7eb;border-radius:20px;padding:22px;box-shadow:0 24px 70px rgba(15,23,42,.25);text-align:center}
          .auro-plan-reset-icon{width:48px;height:48px;margin:0 auto 10px;border-radius:15px;display:grid;place-items:center;background:#fff1f2;color:#be123c;font-size:22px}
          .auro-plan-reset-dialog h5{margin:0 0 7px;font-weight:900;color:#111827}
          .auro-plan-reset-dialog p{margin:0;color:#475569}
          .auro-plan-reset-warning{display:flex;align-items:flex-start;gap:9px;margin-top:14px;padding:10px 12px;border-radius:12px;background:#fff7ed;border:1px solid #fed7aa;color:#9a3412;text-align:left;font-size:13px;font-weight:700;line-height:1.35}
          .auro-plan-reset-id{margin-top:12px;color:#64748b;font-size:11px;word-break:break-all}
          .auro-plan-reset-actions{display:flex;justify-content:flex-end;gap:10px;margin-top:18px}
          .auro-plan-reset-actions .btn{min-height:40px;border-radius:10px;font-weight:800}
          @media(max-width:768px){
            .auro-plan-reset-actions-wrap{display:grid;grid-template-columns:1fr;width:100%}
            .auro-plan-reset-actions-wrap .btn{width:100%}
            .auro-plan-reset-actions{display:grid;grid-template-columns:1fr;gap:8px}
            .auro-plan-reset-actions .btn{width:100%}
            .auro-plan-reset-dialog{padding:18px;border-radius:17px}
          }
        `;
        document.head.appendChild(style);
    }
    ...
    function instalarBotones(){
        ...
        const actualizar = buscarBotonActualizarPlan();
        if(!actualizar) return false;

        let contenedor = actualizar.closest('.auro-plan-reset-actions-wrap');
        if(!contenedor){
            contenedor = document.createElement('div');
            contenedor.className = 'auro-plan-reset-actions-wrap';
            actualizar.parentNode.insertBefore(contenedor, actualizar);
            contenedor.appendChild(actualizar);
        }

        let restablecer = document.getElementById(ID_BOTON);
        if(!restablecer){
            restablecer = document.createElement('button');
            restablecer.type = 'button';
            restablecer.id = ID_BOTON;
            ...
            restablecer.addEventListener('click', abrirModalRestablecer);
            contenedor.insertBefore(restablecer, actualizar);
        }

        let deshacer = document.getElementById(ID_DESHACER);
        if(!deshacer){
            deshacer = document.createElement('button');
            deshacer.type = 'button';
            deshacer.id = ID_DESHACER;
            ...
            deshacer.addEventListener('click', function(){
                restaurarPlanVisible(window.__auroPlanRespaldoRestablecimiento);
            });
            contenedor.insertBefore(deshacer, actualizar);
        }
    ...
    function actualizarBotonesRestablecimiento(){
        const idAtencion = idAtencionSegura();
        const finalizada = idAtencion ? atencionFinalizada(idAtencion) : false;
        const coincide = idAtencion ? contextoCoincidente(idAtencion) : false;

        const restablecer = document.getElementById(ID_BOTON);
        if(restablecer){
            restablecer.disabled = !idAtencion || finalizada || !coincide;
            restablecer.title = finalizada
                ? 'No disponible: la atención está finalizada.'
                : (!coincide ? 'No disponible: el Plan visible no coincide con la atención activa.' : 'Limpiar únicamente el Plan de la atención activa.');
        }

        const deshacer = document.getElementById(ID_DESHACER);
        const respaldo = window.__auroPlanRespaldoRestablecimiento;
        const puedeDeshacer = !!(
            respaldo &&
            idAtencion &&
            String(respaldo.id_atencion || '').trim() === idAtencion &&
            !finalizada &&
            coincide
        );

        if(deshacer){
            deshacer.classList.toggle('d-none', !puedeDeshacer);
        }
    }

    function preparar(){
        if(!instalarBotones()){
            setTimeout(instalarBotones, 300);
        }
        actualizarBotonesRestablecimiento();
    }
    ...
    [
        'aurosanax:atencion-iniciada',
        'aurosanax:atencion-seleccionada',
        'aurosanax:atencion-cambiada',
        'aurosanax:atencion-limpiada'
    ].forEach(nombre => {
        window.addEventListener(nombre, function(){
            window.__auroPlanRespaldoRestablecimiento = null;
            setTimeout(preparar, 0);
            setTimeout(actualizarBotonesRestablecimiento, 250);
        });
    });

    window.auroPlanActualizarBotonRestablecer = actualizarBotonesRestablecimiento;
})();
